...
def _build_coder_brief(
    *,
    ...
    prompt = (
        "Ban la Manager Agent. Tao CODER BRIEF chi tiet bang tieng Viet.\n\n"
        f"## Yeu cau nguoi dung\n{user_request}\n\n"
        f"## Ten skill can tao\n{skill_name}\n\n"
        f"## Plan tu Planner Agent\n{plan_text}\n\n"
        f"## Web context\n{web_context[:1500]}\n\n"
        f"## RAG context\n{rag_context[:800]}\n\n"
        "## Yeu cau CODER BRIEF phai co du 6 muc:\n"
        "1. Muc tieu\n"
        "2. Nguon du lieu\n"
...
def _execute_skill(root: Path, skill_name: str) -> dict:
...
async def run_and_materialize(
    ...
    code_prompt = (
        "Ban la Coder Agent. Sinh code Python cho skill theo CODER BRIEF ben duoi.\n"
        "Chi tra ve code Python thuan, khong markdown, khong giai thich.\n\n"
        f"{_SKILL_CONTRACT}\n\n"
        f"Ten skill: {skill_name}\n\n"
        f"CODER BRIEF:\n{coder_brief}\n\n"
        f"Web context:\n{web_context[:1000]}\n\n"
        "Nhac lai: ham run() PHAI tra ve dict voi keys: "
        "status, collected_at, source_urls, summary."
    )
    try:
        code_text = _generate_text_strict(llm_client, code_prompt)
    except ValueError as exc:
        raise HTTPException(
            status_code=503,
    ...
    if payload.create_test:
        test_prompt = (
            ...
            f"Module: app.skills.dynamic.{skill_name}\n"
            "- from __future__ import annotations\n"
